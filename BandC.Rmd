...
\end{aligned}
$$
which is obviously negative definite. By the second order sufficient condition, our maximum likelihood from earlier is indeed an optimal value.
<!--
...
Now that we have the loglikelihood we can use the EM-algorithm. In our concrete case, the algorithm is simple as stated earlier:
\begin{itemize}
...
$$l(\lambda_0, \lambda_1|z_i,u_i) = \sum_i \log(u_i \lambda_0 \exp(-\lambda_0 z_i)(1 - \exp(-\lambda_1 z_i)) + (1- u_i) \lambda_1 \exp(-\lambda_1 z_i)(1 - \exp(-\lambda_0 z_i)))$$.

This is a rather nasty log-likelihood, and finding an analytic optimum seems impossible. If we were to find a nice expression for the gradient, solving this would most likely require numerics anyways, not to mention the hessian. Therefore we go directly for numerical optimization.
We can use the \texttt{optim} function in R to do the dirtywork for us.
